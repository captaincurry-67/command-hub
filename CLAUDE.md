...
- **Two-signal Discord tracking + forecast-vs-actual, built & fully verified locally (2026-07-24), NOT YET DEPLOYED** — awaiting explicit deploy confirmation + the user's secret setup. Migrations 005/006 applied to local D1; 1,759 events backfilled; `apiServerStats` reads D1 (charts identical); ingest (auth/dedup/validation), scheduled no-op, reconciliation (authoritative + anchorDrift), "N current members" headline, forecast-locking (first-writer-wins), modeled backtest, recorded-overrides-modeled, the 2-bar Forecast-vs-Actual chart + Monthly Summary Forecast/Δ columns — all verified (desktop + 375px, console clean). **Needs at deploy:** migrations 005 + 006 `--remote`, backfill `--remote`, three secrets (`INGEST_SECRET`, `DISCORD_BOT_TOKEN`, `DISCORD_GUILD_ID`) via `wrangler secret put` (user does the token), bot repointed to `/api/member-events` + redeployed on SparkedHost.
